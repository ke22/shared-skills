...
READINESS_REPORT_REL = Path("references/readiness_report.md")
READINESS_REPORT_REFERENCE = READINESS_REPORT_REL.as_posix()
LEGACY_QUALITY_CHECKLIST_REL = Path("references/quality_checklist.md")
LEGACY_QUALITY_CHECKLIST_REFERENCE = LEGACY_QUALITY_CHECKLIST_REL.as_posix()
READINESS_TOPIC_PATTERNS = {
...
def lint_skill(
    ...
    *,
    ...
    readiness_rel = None
    if READINESS_REPORT_REFERENCE in skill_md_text:
        readiness_reference = READINESS_REPORT_REFERENCE
        readiness_rel = READINESS_REPORT_REL
    elif LEGACY_QUALITY_CHECKLIST_REFERENCE in skill_md_text:
        readiness_reference = LEGACY_QUALITY_CHECKLIST_REFERENCE
        readiness_rel = LEGACY_QUALITY_CHECKLIST_REL

    if readiness_reference is not None and readiness_rel is not None:
        readiness_path = skill_dir / readiness_rel
        if not readiness_path.exists():
            findings.append(
                Finding(
                    "ERROR",
                    f"SKILL.md references '{readiness_reference}' but the file is missing",
                    readiness_path,
                )
            )
        else:
            readiness_text = read_text_safely(readiness_path)
            if readiness_text is None:
                findings.append(Finding("ERROR", f"{readiness_reference} is not valid UTF-8 text", readiness_path))
            elif not readiness_text.strip():
                findings.append(Finding("ERROR", f"{readiness_reference} is empty", readiness_path))
            elif profile.check_quality_checklist_topics:
                if not contains_any_pattern(readiness_text, READINESS_TOPIC_PATTERNS["format"]):
                    findings.append(Finding("WARN", f"{readiness_reference} should cover format/structure checks", readiness_path))
                if not contains_any_pattern(readiness_text, READINESS_TOPIC_PATTERNS["requirements"]):
                    findings.append(Finding("WARN", f"{readiness_reference} should cover requirement/policy checks", readiness_path))
                if not contains_any_pattern(readiness_text, READINESS_TOPIC_PATTERNS["common_errors"]):
                    findings.append(Finding("WARN", f"{readiness_reference} should cover common error checks", readiness_path))
                if not contains_any_pattern(readiness_text, READINESS_TOPIC_PATTERNS["final_gate"]):
                    findings.append(Finding("WARN", f"{readiness_reference} should record a final PASS/FAIL/BLOCKED gate", readiness_path))
...
def validate_skill(skill_dir: Path | str, *, profile: ValidationProfile = MINIMAL_PROFILE) -> tuple[bool, str]:
    ...
    return True, "Format smoke check passed; release gate was not run."
